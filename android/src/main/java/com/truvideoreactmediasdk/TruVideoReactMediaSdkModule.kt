...
class TruVideoReactMediaSdkModule(reactContext: ReactApplicationContext) :
    ...
    fun search(
        ...
        scope.launch {
            try {
                Log.d(NAME, "Search started - Page: $page, Type: $type")
                ...
                // Wrap the SDK call in try-catch to prevent crashes
                val response = try {
                ...
                } catch (sdkException: TruvideoSdkException) {
                    Log.e(NAME, "TruvideoSdkException during search: ${sdkException.message}", sdkException)
                    promise?.reject("NETWORK_ERROR", "Unable to fetch media. Please check your internet connection.")
                    return@launch
                } catch (networkException: UnknownHostException) {
                    Log.e(NAME, "Network error - no internet: ${networkException.message}", networkException)
                    promise?.reject("NO_INTERNET", "No internet connection. Please check your connection and try again.")
                    return@launch
                } catch (timeoutException: SocketTimeoutException) {
                    Log.e(NAME, "Request timeout: ${timeoutException.message}", timeoutException)
                    promise?.reject("TIMEOUT", "Request timed out. Please try again.")
                    return@launch
                } catch (ioException: IOException) {
                    Log.e(NAME, "IO Exception: ${ioException.message}", ioException)
                    promise?.reject("NETWORK_ERROR", "Network error occurred. Please check your connection.")
                    return@launch
                }
                ...
                Log.d(NAME, "Search completed - Found ${response.data.size} items")
                promise?.resolve(jsonArray.toString())

            } catch (e: Exception) {
                Log.e(NAME, "Unexpected exception in search: ${e.message}", e)
